...
(function() {
    ...
    baidu.dom.g = function(id) {
        if ("string" == typeof id || id instanceof String)
            return Tdocument.getElementById(id);
    ...
    if (/msie (\d+\.\d)/i.test(navigator.userAgent))
        baidu.browser.ie = baidu.ie = Tdocument.documentMode || +TRegExp["$1"];
    ...
    baidu.dom.removeClass = function(element, className) {
        element = baidu.dom.g(element);
        var oldClasses = element.className.split(/\s+/), newClasses = className.split(/\s+/), lenOld, lenDel = newClasses[Tlength], j, i = 0;
        for (; i < lenDel; ++i)
            for (j = 0,
            lenOld = oldClasses[Tlength]; j < lenOld; ++j)
    ...
    baidu.dom.getPosition = function(element) {
        ...
        if (element.getBoundingClientRect) {
            ...
            } while (parent && parent != element);if (browser.opera > 0 || browser.isWebkit > 0 && getStyle(element, "position") == "absolute")
    ...
    if (/firefox\/(\d+\.\d)/i.test(navigator.userAgent))
        baidu.browser.firefox = +TRegExp["$1"];
    (function() {
        var ua = navigator.userAgent;
        if (/(\d+\.\d)?(?:\.\d)?\s+safari\/?(\d+\.\d+)?/i.test(ua) && !/chrome/i.test(ua))
            baidu.browser.safari = +(TRegExp["$1"] || TRegExp["$2"])
    }
    )();
    if (/chrome\/(\d+\.\d)/i.test(navigator.userAgent))
        baidu.browser.chrome = +TRegExp["$1"];
    ...
    baidu.lang.inherits = function(subClass, superClass, className) {
        var key, proto, selfProps = subClass[Tprototype], clazz = new Function;
    ...
    if (/android (\d+\.\d)/i.test(navigator.userAgent))
        baidu.platform.android = baidu.android = TRegExp["$1"];
    ...
    baidu.lang.Event[Tprototype].inherit = function(e) {
        ...
        if (e.touches) {
            me.touches = [];
            for (var i = 0; i < e.touches[Tlength]; i++)
                me.touches.push({
        ...
        if (e.changedTouches) {
            me.changedTouches = [];
            for (var i = 0; i < e.changedTouches[Tlength]; i++)
        ...
        if (e.targetTouches) {
            me.targetTouches = [];
            for (var i = 0; i < e.targetTouches[Tlength]; i++)
    ...
    baidu.on = baidu.event.on = function(el, type, handler) {
        ...
        if (el[TaddEventListener])
            el[TaddEventListener](type, handler, false);
    ...
    baidu.un = baidu.event.un = function(el, type, handler) {
        if (!(el = baidu.g(el)))
            return el;
        type = type.replace(/^on/, "");
        if (el[TremoveEventListener])
            el[TremoveEventListener](type, handler, false);
    ...
    baidu.enableTapEvent = function() {
        function Tap(element) {
            if (!Tdocument[TaddEventListener])
                return;
        ...
        Tap[Tprototype] = {
            ...
            move: function(e) {
                ...
                if (TMath.abs(x - this.startX) > 10 || TMath.abs(y - this.startY) > 10)
                    this.moved = true
            },
            end: function(e) {
                stopAndPrevent(e);
    ...
    ;
    BMap.apiLoad = BMap["apiLoad"] || function() {}
    ;
    ...
    function Map(element, opts) {
        ...
        if (opts["enableAutoResize"])
            me.config.enableAutoResize = opts["enableAutoResize"];
        ...
        if (mapType === BMAP_PERSPECTIVE_MAP)
            _addStat(5002);
        if (mapType === BMAP_SATELLITE_MAP || mapType === BMAP_HYBRID_MAP)
            _addStat(5003);
        ...
        for (var i = 0; i < BMap._register[Tlength]; i++)
            BMap._register[i](me);
        ...
        if (isDesktop())
            ModuleMgr.load("oppc", function() {
                me._asyncRegister()
            });
        if (isMobile())
            ModuleMgr.load("opmb", function() {
                me._asyncRegister()
            });
        element = null;
        me._viewTiles = []
    ...
    baidu[Textend](Map[Tprototype], {
        ...
        _bind: function() {
            ...
            ;
            if (me.config.enableAutoResize)
                me.temp.autoResizeTimer = setInterval(me._watchSize, 80)
        },
        ...
        zoomTo: function(level, point) {
            ...
            if (point)
                ...
                cPoint = this.getInfoWindow().getPosition();
            if (cPoint) {
        ...
        panTo: function(center, opts) {
            ...
            if (Point.isInRange(center))
                this.centerPoint = new Point(center["lng"],center["lat"]);
            else
        ...
        addTileLayer: function(tilelayer) {
            if (tilelayer)
                this.dispatchEvent(new BaseEvent("onaddtilelayer",tilelayer))
        },
        removeTileLayer: function(tilelayer) {
            if (tilelayer)
                this.dispatchEvent(new BaseEvent("onremovetilelayer",tilelayer))
        },
            ...
            if (center instanceof Point)
                ...
                local.setSearchCompleteCallback(function(result) {
                    if (local.getStatus() == 0 && local._json["result"]["type"] == 2) {
                        ...
                        if (BMAP_PERSPECTIVE_MAP.getCityName(center))
                            me.setCurrentCity(center)
                    }
                });
                local.search(center, {
                    log: "center"
        ...
        centerAndZoom: function(center, zoom) {
            ...
            if (!(center instanceof Point) || !zoom)
                return;
            ...
            if (!me.loaded) {
                me.loaded = true;
                me.dispatchEvent(e);
                if (!TIME_LINE.loadedMap)
                    TIME_LINE.loadedMap = getCurrentTime()
            }
            me.dispatchEvent(ecode);
            me.dispatchEvent(new BaseEvent("onmoveend"));
            if (me.lastLevel != me.zoomLevel)
                me.dispatchEvent(new BaseEvent("onzoomend"))
        },
        _getLocal: function() {
            if (!this.temp.local)
        ...
        getSize: function() {
            if (this.customSize && this.customSize instanceof Size)
                return new Size(this.customSize[Twidth],this.customSize[Theight]);
            else
        ...
        "pointToPixelFor3D": function(point, callback) {
            ...
            if (mapType == BMAP_PERSPECTIVE_MAP && curCity)
                CoordTransUtils.convertPtToPix3D(point, me, callback)
        },
        ...
        _getBestLevel: function(bounds, opts) {
            ...
            for (var level = maxLevel; level >= minLevel; level--) {
                ...
                if (bounds.toSpan()["lng"] / zoomUnits < this[Twidth] - ew && bounds.toSpan()["lat"] / zoomUnits < this[Theight] - eh)
                    break
            }
            level += zoomFactor;
            if (level < minLevel)
        ...
        getViewport: function(view, opts) {
            ...
            if (!view || !view instanceof Bounds && view[Tlength] == 0 || view instanceof Bounds && view.isEmpty())
                return curViewport;
            var points = [];
            ...
            opts = opts || {};
            var mercatorPts = [];
            for (var i = 0, l = points[Tlength]; i < l; i++)
                mercatorPts.push(this.projection.lngLatToMercator(points[i], this.currentCity));
            var bounds = new Bounds;
            for (var i = mercatorPts[Tlength] - 1; i >= 0; i--)
                bounds[Textend](mercatorPts[i]);
            if (bounds.isEmpty())
                return curViewport;
        ...
        setViewport: function(view, opts) {
            ...
            if (view && view["center"])
                ...
                viewport = this.getViewport(view, opts);
            opts = opts || {};
            ...
            if (viewport.zoom == this.zoomLevel && opts["enableAnimation"] != false) {
                ...
                this.centerAndZoom(viewport["center"], viewport["zoom"])
        },
        getPanes: function() {
            return this._panes
        },
        ...
        getOverlays: function() {
            ...
            if (overlays)
                for (var i in overlays)
                    if (overlays[i]instanceof OverlayInternal)
                        results.push(overlays[i]);
            if (customOverlays)
                for (var i = 0, l = customOverlays[Tlength]; i < l; i++)
                    results.push(customOverlays[i]);
            return results
        },
        getMapType: function() {
            return this.mapType
        ...
        _asyncRegister: function() {
            for (var i = this.temp.registerIndex; i < BMap._register[Tlength]; i++)
                BMap._register[i](this);
        ...
        setCurrentCity: function(city) {
            ...
            if (me.mapType == BMAP_PERSPECTIVE_MAP)
                if (me.projection instanceof PerspectiveProjection)
                    me.projection.curCityName = me.currentCity
        },
        ...
        removeHotspot: function(hotspot, tag) {
            if (!tag)
                tag = "custom";
            if (this._hotspots[tag][hotspot.guid])
                delete this._hotspots[tag][hotspot.guid]
        },
        ...
        _checkZoom: function() {
            ...
            if (config.minZoom < mapTypeMinZoom)
                config.minZoom = mapTypeMinZoom;
            if (config.maxZoom > mapTypeMaxZoom)
        ...
        _updateZoom: function() {
            ...
            else if (this.zoomLevel > config.maxZoom)
                this.setZoom(config.maxZoom);
    ...
    Animation[Tprototype]._launch = function() {
        ...
        if (now >= me._endTime) {
            if (isFunction(me._opts.render))
                me._opts.render(me._opts.transition(1));
            if (isFunction(me._opts.finish))
                me._opts.finish();
    ...
    Animation[Tprototype].setFinishCallback = function(callback) {
        if (this._anis[Tlength] > 0)
            this._anis[this._anis[Tlength] - 1]._opts.finish = callback;
        else
    ...
    var Transitions = {
        ...
        easeInOutCubic: function(t) {
            if (t < 0.5)
                return TMath.pow(t, 3) * 4;
            else
                return TMath.pow(t - 1, 3) * 4 + 1
        },
    ...
    var MapConfig = {
        imgPath: "https://api.map.baidu.com/images/",
        cityNames: {
            "\u5317\u4eac": {
                pinyin: "bj",
    ...
    if (baidu.browser.firefox) {
        ...
        if (baidu.platform.isWindows)
            MapConfig.fontFamily = "arial,simsun,sans-serif"
    } else if (baidu.browser.chrome || baidu.browser.safari)
        baidu[Textend](MapConfig, {
    ...
    function create(tag, attr, ns) {
        ...
        if (ns)
            e = Tdocument.createElementNS(ns, tag);
    ...
    function decode64(input) {
        ...
        if (!input || base64test.exec(input))
            return input;
        ...
        do {
            ...
            chr1 = chr2 = chr3 = "";
            enc1 = enc2 = enc3 = enc4 = ""
        } while (i < input[Tlength]);return output
    }
    ...
    function isSvg() {
        return !!Tdocument.implementation.hasFeature("https://www.w3.org/TR/SVG11/feature#Shape", "1.1")
    }
    function isCanvas() {
    ...
    ;var DataMgr = {
        request: function(url) {
            ...
            if (script[TaddEventListener])
                script[TaddEventListener]("load", function(e) {
                    var t = e.target;
    ...
    var ModuleVersion = {
        "map": "20201225071949",
        "tile": "20201225071949",
        "marker": "20201225071949",
        "markeranimation": "20201225071949",
        "poly": "20201225071949",
        "draw": "20201225071949",
        "drawbysvg": "20201225071949",
        "drawbyvml": "20201225071949",
        "drawbycanvas": "20201225071949",
        "infowindow": "20201225071949",
        "oppc": "20201225071949",
        "opmb": "20201225071949",
        "menu": "20201225071949",
        "control": "20201225071949",
        "navictrl": "20201225071949",
        "geoctrl": "20201225071949",
        "copyrightctrl": "20201225071949",
        "scommon": "20201225071949",
        "local": "20201225071949",
        "route": "20201225071949",
        "othersearch": "20201225071949",
        "buslinesearch": "20201225071949",
        "hotspot": "20201225071949",
        "autocomplete": "20201225071949",
        "coordtrans": "20201225071949",
        "coordtransutils": "20201225071949",
        "mapclick": "20201225071949"
    };
    baidu.storage = function() {
        ...
        function _remove(moduleName) {
            var prefix = _prefix + moduleName + "_", l = ls[Tlength], strKey;
            while (l--) {
                strKey = ls.key(l);
                if (strKey.indexOf(prefix) > -1)
                    ls.removeItem(strKey)
            }
        }
        function _get(moduleName) {
            if (_isSupportLocalStorage && _check(moduleName))
                return ls.getItem(_getRealyModuleName(moduleName));
            else
                return false
        }
        return {
    ...
    baidu.object[Textend](ModuleMgr, {
        ...
        preLoaded: {},
        Config: {
            _baseUrl: "https://api.map.baidu.com/getmodules?v=1.4",
            _timeout: 5E3
        },
        delayFlag: false,
        Module: {
        ...
        combine: function(moduleName) {
            if (moduleName && this.getDependency()[moduleName]) {
                ...
                for (var i = 0; i < dep[Tlength]; i++) {
                    this.combine(dep[i]);
                    if (!this.Module._modules[dep[i]])
                        this.pushUniqueMdl(dep[i])
                }
            }
        },
        pushUniqueMdl: function(mdl) {
            for (var i = 0; i < this.Module._arrMdls[Tlength]; i++)
                if (this.Module._arrMdls[i] == mdl)
                    return;
        ...
        run: function(moduleName, moduleCode) {
            ...
            for (var i = 0, l = currentModule._callbacks[Tlength]; i < l; i++)
                currentModule._callbacks[i]();
        ...
        checkCodesStatus: function(mdlName, mdlCode) {
            ...
            for (var i = 0, l = tempMdl[Tlength]; i < l; i++)
                if (tempMdl[i].mdlCode == "")
                    if (tempMdl[i].mdlName == mdlName)
                        tempMdl[i].mdlCode = mdlCode;
                    else
                        return;
            this.evalAllCode()
        },
        evalAllCode: function() {
            var tempMdl = this.Module._tempMdls;
            for (var i = 0, l = tempMdl[Tlength]; i < l; i++)
                this.run(tempMdl[i].mdlName, tempMdl[i].mdlCode);
            this.Module._tempMdls[Tlength] = 0
        }
    });
    function _jsload(mdlName, mdlCode) {
        if (T.storage.isSupportLocalStorage)
            T.storage.set(mdlName, mdlCode);
    ...
    baidu[Textend](Hotspot[Tprototype], {
        initialize: function(map) {
            if (this._minZoom == null)
                this._minZoom = map.config.minZoom;
            if (this._maxZoom == null)
                this._maxZoom = map.config.maxZoom
        },
    ...
    baidu[Textend](Control[Tprototype], {
        ...
        _i: function(map) {
            if (!this._container && this["initialize"] && isFunction(this["initialize"]))
        ...
        _setStyle: function() {
            ...
            if (c) {
                ...
                if (!this._opts.printable)
                    baidu.dom.addClass(c, "BMap_noprint");
                if (!isMobile())
                    baidu.on(c, "contextmenu", stopAndPrevent)
            }
        },
        remove: function() {
            this._map = null;
        ...
        _render: function() {
            ...
            if (this._visible == false)
                baidu.dom.hide(this._container);
            return this._container
        },
        ...
        setAnchor: function(anchor) {
            if (this.anchorFixed || !isNumber(anchor) || isNaN(anchor) || anchor < BMAP_ANCHOR_TOP_LEFT || anchor > BMAP_ANCHOR_BOTTOM_RIGHT)
        ...
        show: function() {
            ...
            this._visible = true;
            if (this._container)
                baidu.dom.show(this._container)
        },
        hide: function() {
            if (this._visible == false)
                return;
    ...
    baidu[Textend](NavigationControl[Tprototype], {
        ...
        setType: function(type) {
            if (isNumber(type) && type >= BMAP_NAVIGATION_CONTROL_LARGE && type <= BMAP_NAVIGATION_CONTROL_ZOOM)
    ...
    baidu.object[Textend](CopyrightControl[Tprototype], {
        ...
        addCopyright: function(copyright) {
            if (!copyright || !isNumber(copyright["id"]) || isNaN(copyright["id"]))
                return;
            var ncpy = {
                "bounds": null,
                "content": ""
            };
            for (var i in copyright)
                ncpy[i] = copyright[i];
            ...
            if (cp)
                ...
                this._copyrightCollection.push(ncpy)
        },
        getCopyright: function(id) {
            for (var i = 0, l = this._copyrightCollection[Tlength]; i < l; i++)
                if (this._copyrightCollection[i]["id"] == id)
                    return this._copyrightCollection[i]
        },
    ...
    function MapTypeControl(opts) {
        ...
        this.setAnchor(opts["anchor"]);
        if (isArray(opts["mapTypes"]))
            this._opts.mapTypes = opts["mapTypes"].slice(0);
        this._asyncLoadCode()
    }
    ...
    baidu.object[Textend](ContextMenu[Tprototype], {
        ...
        addItem: function(item) {
            if (!item || item._type != "menuitem" || item._text == "" || item._width <= 0)
                return;
        ...
        removeItem: function(item) {
            ...
            for (var i = 0, l = this._rItems[Tlength]; i < l; i++)
                if (this._rItems[i] === item) {
    ...
    baidu.object[Textend](Bounds[Tprototype], {
        ...
        equals: function(other) {
            if (!(other instanceof Bounds) || this.isEmpty())
                return false;
        ...
        containsBounds: function(bounds) {
            if (!(bounds instanceof Bounds) || this.isEmpty() || bounds.isEmpty())
                return false;
        ...
        intersects: function(bounds) {
            if (!(bounds instanceof Bounds))
                return null;
            if (TMath.max(bounds._swLng, bounds._neLng) < TMath.min(this._swLng, this._neLng) || TMath.min(bounds._swLng, bounds._neLng) > TMath.max(this._swLng, this._neLng) || TMath.max(bounds._swLat, bounds._neLat) < TMath.min(this._swLat, this._neLat) || TMath.min(bounds._swLat, bounds._neLat) > TMath.max(this._swLat, this._neLat))
                return null;
        ...
        containsPoint: function(point) {
            if (!(point instanceof Point) || this.isEmpty())
                return false;
    ...
    baidu[Textend](MercatorProjection, {
        ...
        convertMC2LL: function(point) {
            ...
            for (var i = 0; i < this.MCBAND[Tlength]; i++)
        ...
        convertLL2MC: function(point) {
            ...
            for (var i = 0; i < this.LLBAND[Tlength]; i++)
                if (temp["lat"] >= this.LLBAND[i]) {
                    factor = this.LL2MC[i];
                    break
                }
            if (!factor)
                for (var i = this.LLBAND[Tlength] - 1; i >= 0; i--)
                    if (temp["lat"] <= -this.LLBAND[i]) {
                        factor = this.LL2MC[i];
                        break
                    }
    ...
    baidu[Textend](Overlay[Tprototype], {
        ...
        remove: function() {
            if (this.domElement && this.domElement[TparentNode])
                this.domElement[TparentNode][TremoveChild](this.domElement);
            this.domElement = null;
            this.dispatchEvent(new BaseEvent("onremove"))
        },
        hide: function() {
            if (this.domElement)
                baidu.dom.hide(this.domElement)
        },
        show: function() {
            if (this.domElement)
                baidu.dom.show(this.domElement)
        },
        isVisible: function() {
            if (!this.domElement)
                return false;
    ...
    BMap.register(function(map) {
        ...
        if (baidu.browser.ie && baidu.browser.ie < 8 || Tdocument.compatMode == "BackCompat")
            map[TaddEventListener]("zoomend", function(e) {
                setTimeout(function() {
                    mgr["draw"](e)
                }, 20)
            });
        ...
        map[TaddEventListener]("addoverlay", function(e) {
            ...
            if (overlay instanceof OverlayInternal) {
                if (!mgr._overlays[overlay.guid])
                    mgr._overlays[overlay.guid] = overlay
            } else {
                var exist = false;
                for (var i = 0, l = mgr._customOverlays[Tlength]; i < l; i++)
                    if (mgr._customOverlays[i] === overlay) {
                        exist = true;
                        break
                    }
                if (!exist)
                    mgr._customOverlays.push(overlay)
            }
        });
        map[TaddEventListener]("removeoverlay", function(e) {
            var overlay = e.target;
            if (overlay instanceof OverlayInternal)
                delete mgr._overlays[overlay.guid];
            else
                for (var i = 0, l = mgr._customOverlays[Tlength]; i < l; i++)
                    if (mgr._customOverlays[i] === overlay) {
        ...
        map[TaddEventListener]("clearoverlays", function(e) {
            ...
            for (var overlayHashCode in mgr._overlays)
                if (mgr._overlays[overlayHashCode]._config.enableMassClear) {
                    mgr._overlays[overlayHashCode].remove();
                    delete mgr._overlays[overlayHashCode]
                }
            for (var i = 0, l = mgr._customOverlays[Tlength]; i < l; i++)
                if (mgr._customOverlays[i].enableMassClear != false) {
    ...
    OverlayMgr[Tprototype]["draw"] = function(e) {
        for (var overlayHashCode in this._overlays)
            this._overlays[overlayHashCode]["draw"]();
    ...
    Graph.getGraphPoints = function(points) {
        ...
        if (Object[Tprototype].toString.apply(points) == "[object Array]" && points[Tlength] > 0)
            result = points;
        return result
    }
    ;
    ...
    baidu[Textend](Graph[Tprototype], {
        ...
        setStrokeWeight: function(weight) {
            if (weight > 0)
                this._config.strokeWeight = weight
        },
        ...
        setStrokeStyle: function(strokeStyle) {
            if (strokeStyle != "solid" && strokeStyle != "dashed")
                return;
        ...
        remove: function() {
            if (this.map)
                this.map[TremoveEventListener]("onmousemove", this._graphMouseEvent);
            OverlayInternal[Tprototype].remove.call(this);
            this._parseCache[Tlength] = 0
        },
        enableEditing: function() {
            if (this.points[Tlength] < 2)
                return;
    ...
    baidu[Textend](Division[Tprototype], {
        _addDom: function() {
            ...
            if (config.borderColor != "none")
                html.push("border:" + config.lineStroke + "px " + config.lineStyle + " " + config.lineColor + ";");
    ...
    function InfoWindow(content, opts) {
        ...
        if (this._config[Twidth] != 0) {
            if (this._config[Twidth] < 220)
                this._config[Twidth] = 220;
            if (this._config[Twidth] > 730)
                this._config[Twidth] = 730
        }
        if (this._config[Theight] != 0) {
            if (this._config[Theight] < 60)
                this._config[Theight] = 60;
            if (this._config[Theight] > 650)
                this._config[Theight] = 650
        }
    ...
    Map[Tprototype].openInfoWindow = function(infoWin, point) {
        if (!(infoWin instanceof InfoWindow) || !(point instanceof Point))
            return;
        var t = this.temp;
        if (!t.marker) {
    ...
    Map[Tprototype].closeInfoWindow = function() {
        ...
        if (infoWin && infoWin.overlay)
            infoWin.overlay.closeInfoWindow()
    }
    ;
    ...
    function Label(content, opts) {
        ...
        if (this._config[Twidth] < 0)
            this._config[Twidth] = 0;
        if (isDefined(opts["enableClicking"]))
            this._config.clickable = opts["enableClicking"];
    ...
    baidu[Textend](Label[Tprototype], {
        ...
        setPosition: function(point) {
            if (point instanceof Point && !this.getMarker())
                this.point = this._config[Tposition] = new Point(point["lng"],point["lat"])
        },
        setContent: function(content) {
            this.content = content
        },
        ...
        setMarker: function(marker) {
            this._marker = marker;
            if (marker)
                this.point = this._config[Tposition] = marker.getPosition();
            else
                this.point = this._config[Tposition] = null
        },
        getMarker: function() {
    ...
    function Marker(point, opts) {
        ...
        if (opts["icon"] && !opts["shadow"])
            this._config.shadow = null;
        if (opts["enableDragging"])
            this._config.enableDragging = opts["enableDragging"];
        if (opts["enableMassClear"])
            this._config.enableMassClear = opts["enableMassClear"];
        if (isDefined(opts["enableClicking"]))
            this._config.clickable = opts["enableClicking"];
        var me = this;
        ModuleMgr.load("marker", function() {
    ...
    baidu[Textend](Marker[Tprototype], {
        ...
        setShadow: function(shadow) {
            if (shadow instanceof Icon)
                this._config.shadow = shadow
        },
        "getShadow": function() {
        ...
        setPosition: function(point) {
            if (point instanceof Point)
                this.point = new Point(point["lng"],point["lat"])
        },
    ...
    function Polygon(points, opts) {
        ...
        if (opts.fillColor == "")
            ...
            this._config.fillColor = opts["fillColor"] ? opts["fillColor"] : "#fff";
        this.setPath(points);
        var me = this;
        ModuleMgr.load("poly", function() {
            me._draw()
    ...
    baidu[Textend](Polygon[Tprototype], {
        setPath: function(points, flag) {
            ...
            if (pts[Tlength] > 1 && !pts[0].equals(pts[pts[Tlength] - 1]))
                pts.push(new Point(pts[0]["lng"],pts[0]["lat"]));
            Graph[Tprototype].setPath.call(this, pts, flag)
        },
        setPositionAt: function(index, point) {
            if (!this._userPoints[index])
                return;
            this._userPoints[index] = new Point(point["lng"],point["lat"]);
            this.points[index] = new Point(point["lng"],point["lat"]);
            if (index == 0 && !this.points[0].equals(this.points[this.points[Tlength] - 1]))
                this.points[this.points[Tlength] - 1] = new Point(point["lng"],point["lat"]);
            this._calcBounds()
        },
        getPath: function() {
    ...
    baidu[Textend](TileMgr[Tprototype], {
        ...
        showTile: function(tileInfo, centerPos, tilelayer) {
            ...
            var rpix = 0;
            if (mapType === BMAP_PERSPECTIVE_MAP && me.map.getZoom() == 15)
                rpix = 0.5;
        ...
        getTileName: function(tileInfo, tilelayer) {
            if (this.map.getMapType() === BMAP_PERSPECTIVE_MAP)
                return "TILE-" + tilelayer.guid + "-" + this.map.cityCode + "-" + tileInfo[0] + "-" + tileInfo[1] + "-" + tileInfo[2];
            else
                return "TILE-" + tilelayer.guid + "-" + tileInfo[0] + "-" + tileInfo[1] + "-" + tileInfo[2]
        },
        ...
        _moveGridTiles: function() {
            ...
            for (var n = 0; n < layernums; n++) {
                ...
                if (tilelayer.baseLayer)
                    this.tilesDiv = tilelayer.tilesDiv;
                var map = this.map;
                ...
                if (mapType == BMAP_PERSPECTIVE_MAP)
                    if (centerPoint.equals(new Point(0,0)))
                        centerPoint = map.mercatorCenter = projection.lngLatToMercator(map.centerPoint, map.currentCity);
                ...
                for (var tileName in mapTiles) {
                    ...
                    var info = tile.info;
                    if (info[2] != zoom || info[2] == zoom && (fromRow > info[0] || toRow <= info[0] || fromColumn > info[1] || toColumn <= info[1]))
                        this.hideTile(tile)
                }
                ...
                for (var i = 0, l = tilesOrder[Tlength]; i < l; i++)
                    this.showTile([tilesOrder[i][0], tilesOrder[i][1], zoom], centerPosition, tilelayer)
            }
            return
        },
        addTileLayer: function(e) {
            ...
            for (var i = 0; i < me.tileLayers[Tlength]; i++)
                if (me.tileLayers[i] == tilelayer)
                    return;
        ...
        removeTileLayer: function(e) {
            var me = this;
            var tilelayer = e.target;
            for (var i = 0, l = me.tileLayers[Tlength]; i < l; i++)
                if (tilelayer == me.tileLayers[i])
                    me.tileLayers.splice(i, 1);
            tilelayer.remove()
        },
        setMapType: function() {
            var me = this;
            var layers = this.mapTypeLayers;
            for (var i = 0, l = layers[Tlength]; i < l; i++)
                layers[i].remove();
    ...
    function Tile(mgr, src, position, tileInfo, tilelayer) {
        ...
        this.img.onload = function(e) {
            ...
            if (me.img && !isInDocument(me.img))
                if (tilelayer.tilesDiv) {
                    tilelayer.tilesDiv[TappendChild](me.img);
                    if (baidu.browser.ie <= 6 && baidu.browser.ie > 0 && me._transparentPng)
                        me.img.style.cssText += ';filter: progid:DXImageTransform.Microsoft.AlphaImageLoader(src="' + me.src + '",sizingMethod=scale);'
                }
            ...
            if (fadeInTile)
                new Animation({
                    ...
                    duration: 200,
                    render: function(s) {
                        if (me.img && me.img.style)
                            me.img.style.opacity = s * 1
                    },
                    finish: function() {
                        if (me.img && me.img.style)
                            delete me.img.style.opacity
                    }
                });
            me._callCbks()
        }
    ...
    Tile[Tprototype]._load = function() {
        if (baidu.browser.ie > 0 && baidu.browser.ie <= 6 && this._transparentPng)
            this.img.src = MapConfig.imgPath + "blank.gif";
        else
    ...
    Tile[Tprototype]._callCbks = function() {
        var me = this;
        for (var i = 0; i < me._cbks[Tlength]; i++)
            me._cbks[i]();
        me._cbks[Tlength] = 0
    }
    ;
    ...
    function TrafficLayer(opts) {
        ...
        this._tileUrl = "https://its.map.baidu.com:8002/traffic/"
    }
    ...
    TrafficLayer[Tprototype]["getTilesUrl"] = function(point, level) {
        var pDate = "";
        if (this._opts["predictDate"])
            pDate = "HistoryService?" + "day=" + (this._opts["predictDate"]["weekday"] - 1) + "&hour=" + this._opts["predictDate"]["hour"] + "&t=" + (new Date).getTime() + "&";
        else {
            pDate = "TrafficTileService?" + "time=" + (new Date).getTime() + "&";
            if (!this._map.highResolutionEnabled())
                pDate += "label=web2D&v=016&"
        }
        var map = this._map
          , row = point.x
          , column = point.y
    ...
    var TILE_BASE_URLS = ["https://shangetu0.map.bdimg.com/it/", "https://shangetu1.map.bdimg.com/it/", "https://shangetu2.map.bdimg.com/it/", "https://shangetu3.map.bdimg.com/it/", "https://shangetu4.map.bdimg.com/it/"];
    var TILE_ONLINE_URLS = ["https://maponline0.bdimg.com/tile/", "https://maponline1.bdimg.com/tile/", "https://maponline2.bdimg.com/tile/", "https://maponline3.bdimg.com/tile/"];
    ...
    normalMapTileLayer["getTilesUrl"] = function(tileCoord, zoom) {
        ...
        if (this.map.highResolutionEnabled())
            styles = "ph";
        var url = TILE_ONLINE_URLS[TMath.abs(row + column) % TILE_ONLINE_URLS[Tlength]] + "?qt=vtile&x=" + (row + "").replace(/-/gi, "M") + "&y=" + (column + "").replace(/-/gi, "M") + "&z=" + zoom + "&styles=" + styles + (baidu.browser.ie == 6 ? "&color_dep=32&colors=50" : "") + "&udt=" + udt;
        return url.replace(/-(\d+)/gi, "M$1")
    }
    ;
    ...
    PerspectiveTileLayer.tileUrls = ["https://d0.map.baidu.com/resource/mappic/", "https://d1.map.baidu.com/resource/mappic/", "https://d2.map.baidu.com/resource/mappic/", "https://d3.map.baidu.com/resource/mappic/"];
    ...
    BMAP_PERSPECTIVE_MAP.getCityName = function(cname) {
        ...
        for (var cn in cns)
            if (cname.search(cn) > -1)
                return cns[cn].pinyin;
        return ""
    }
    ;
    ...
    function BaseSearch(loc, opts) {
        ...
        this._opts = {
            ...
            onSearchComplete: opts["onSearchComplete"] || function() {}
            ,
            onMarkersSet: opts["onMarkersSet"] || function() {}
            ,
            onInfoHtmlSet: opts["onInfoHtmlSet"] || function() {}
            ,
            onResultsHtmlSet: opts["onResultsHtmlSet"] || function() {}
            ,
            onGetBusListComplete: opts["onGetBusListComplete"] || function() {}
            ,
            onGetBusLineComplete: opts["onGetBusLineComplete"] || function() {}
            ,
            onBusListHtmlSet: opts["onBusListHtmlSet"] || function() {}
            ,
            onBusLineHtmlSet: opts["onBusLineHtmlSet"] || function() {}
            ,
            onPolylinesSet: opts["onPolylinesSet"] || function() {}
            ,
            reqFrom: opts["reqFrom"] || ""
        };
        if (typeof opts != "undefined" && typeof opts["renderOptions"] != "undefined" && typeof opts["renderOptions"]["autoViewport"] != "undefined")
            this._opts.renderOptions.autoViewport = opts["renderOptions"]["autoViewport"];
        else
    ...
    baidu[Textend](BaseSearch[Tprototype], {
        ...
        setSearchCompleteCallback: function(cbk) {
            this._opts.onSearchComplete = cbk || function() {}
        },
        "setMarkersSetCallback": function(cbk) {
            this._opts.onMarkersSet = cbk || function() {}
        },
        "setPolylinesSetCallback": function(cbk) {
            this._opts.onPolylinesSet = cbk || function() {}
        },
        "setInfoHtmlSetCallback": function(cbk) {
            this._opts.onInfoHtmlSet = cbk || function() {}
        },
        "setResultsHtmlSetCallback": function(cbk) {
            this._opts.onResultsHtmlSet = cbk || function() {}
        },
        getStatus: function() {
            return this._status
        }
    ...
    var SearchRequestMgr = {
        REQ_BASE_URL: "https://api.map.baidu.com/",
        request: function(cbk, params, userData, path, disableLog) {
            ...
            path = path || "";
            var reqParam;
            if (userData && userData.useEncodeURI)
                reqParam = jsonToQuery(params, encodeURI);
            else
                reqParam = jsonToQuery(params, encodeURIComponent);
            var me = this
    ...
    var MAP_URL = "https://map.baidu.com/";
    var API_URL = "https://api.map.baidu.com/";
    ...
    baidu[Textend](TransitRoute[Tprototype], {
        setPolicy: function(policy) {
            if (policy >= BMAP_TRANSIT_POLICY_LEAST_TIME && policy <= BMAP_TRANSIT_POLICY_AVOID_SUBWAYS)
        ...
        setPageCapacity: function(cp) {
            ...
            if (cp >= 1 && cp <= TransitRoute.MAX_PAGE_CAPACITY)
                this._opts.pageCapacity = TMath.round(cp);
            else
                this._opts.pageCapacity = TransitRoute.MAX_PAGE_CAPACITY
        },
        toString: function() {
    ...
    var DWRoute = function(location, opts) {
        ...
        if (renderOptions.highlightMode != BMAP_HIGHLIGHT_STEP && renderOptions.highlightMode != BMAP_HIGHLIGHT_ROUTE)
            renderOptions.highlightMode = BMAP_HIGHLIGHT_STEP;
    ...
    baidu[Textend](BusLineSearch[Tprototype], {
        ...
        "setGetBusListCompleteCallback": function(cbk) {
            this._opts.onGetBusListComplete = cbk || function() {}
        },
        "setGetBusLineCompleteCallback": function(cbk) {
            this._opts.onGetBusLineComplete = cbk || function() {}
        },
        "setBusListHtmlSetCallback": function(cbk) {
            this._opts.onBusListHtmlSet = cbk || function() {}
        },
        "setBusLineHtmlSetCallback": function(cbk) {
            this._opts.onBusLineHtmlSet = cbk || function() {}
        },
        "setPolylinesSetCallback": function(cbk) {
            this._opts.onPolylinesSet = cbk || function() {}
        }
